...
static cmdline_processor::register_flag
    cmd_emit_cppfront_info(9, "emit-cppfront-info",
                           "Emit cppfront version/build in output file",
                           [] { flag_emit_cppfront_info = true; });

static auto flag_clean_cpp1 = false;
static cmdline_processor::register_flag
    cmd_clean_cpp1(9, "clean-cpp1", "Emit clean Cpp1 without #line directives",
                   [] { flag_clean_cpp1 = true; });

static auto flag_line_paths = false;
static cmdline_processor::register_flag
    cmd_line_paths(9, "line-paths", "Emit absolute paths in #line directives",
...
static cmdline_processor::register_flag
    cmd_verbose(9, "verbose", "Print verbose statistics and -debug output",
                [] { flag_verbose = true; });
...
private:
  ...
  auto print(std::string_view s, source_position pos = source_position{},
    ...
    *out << s;
    ...
    if (track_curr_pos) {
      auto last_newline =
          std::string::npos; // the last newline we found in the string
      auto newline_pos =
...
public:
  ...
  //  Open
  //
  auto open(std::string cpp2_filename_, std::string cpp1_filename_,
...
#include "build.info"
    ...
    auto print_cpp2util = [&]() {
      if (!tokens.get_map().empty() || flag_import_std || flag_include_std) {
        printer.print_extra("\n#include \"cpp2util.h\"\n\n");
      }
    };
  ...
  auto emit(unqualified_id_node const &n, int synthesized_multi_return_size = 0,
    ...
    bool add_move =
        !add_forward && (!last_use || last_use->safe_to_move) &&
    ...
    bool add_this = add_move && decl && decl->identifier &&
                    *decl->identifier == "this" && *n.identifier != "this";
  ...
  auto emit(return_statement_node const &n) -> void {
    ...
    if (n.expression) {
      ...
      if (auto tok = n.expression->expr->get_postfix_expression_node()
                         ->expr->get_token();
          tok && is_forward_return) {
        ...
        if (is_parameter_name &&
          ...
          return;
        } else if (is_literal(tok->type()) ||
                   n.expression->expr->is_result_a_temporary_variable()) {
          errors.emplace_back(
              n.position(),
    ...
    //
      ...
      for (bool first = true; auto &param : parameters) {
        ...
        emit(*param->declaration->identifier,
             cpp2::unsafe_narrow<int>(parameters.size()));
        printer.emit_to_string();
      }
  ...
  auto is_pointer_declaration(type_id_node const *type_id_node, int deref_cnt,
    ...
    return (unsafe_narrow<int>(pointer_declarators_cnt) + addr_cnt -
            deref_cnt) > 0;
  }
      ...
      //
        ...
        if (flag_safe_subscripts && i->op->type() == lexeme::LeftBracket &&
          ...
          if (auto lit = i->expr_list->expressions.front().expr->get_literal();
              lit && lit->literal->type() == lexeme::DecimalLiteral) {
    ...
    for (auto i = n.ops.rbegin(); i != n.ops.rend(); ++i) {
      ...
      if (i->type) {
        if (i->type->is_wildcard()) {
          ...
          prefix +=
              "cpp2::impl::" + op_name + "<" + print_to_string(*i->type) + ">(";
          suffix = ")" + suffix;
        }
      }
  ...
  //
  auto check_shadowing_of_type_scope_names(declaration_node const &decl)
      -> bool {
    if (decl.has_name()           // this is a named declaration
        && !decl.has_name("this") // that's not 'this'
        && !decl.parent_is_type() // and the type isn't the direct parent
        && is_name_declared_in_current_type_scope(
               *decl.name())) // and it shadows a name
    {
      errors.emplace_back(decl.position(),
                          "a type's implementation may not declare a name that "
                          "is the same as (i.e., shadows) a type scope name - "
                          "for example, a type scope function's local variable "
                          "may not have the same as one of the type's members");
      return false;
    }
  ...
  auto emit(function_type_node const &n, bool is_main = false,
            bool is_ctor_or_dtor = false, std::string suffix1 = {},
            bool generating_postfix_inc_dec = false) -> void {
    STACKINSTR
    if (!sema.check(n)) {
      return;
    ...
    if (n.is_assignment() || generating_assignment_from == n.my_decl) {
    ...
    //  Otherwise, handle a single anonymous return type
      ...
      auto is_type_scope_function_with_in_this =
          n.my_decl->parent_is_type() && n.parameters->ssize() > 0 &&
  ...
  auto emit(declaration_node const &n, std::string const &capture_intro = {})
    ...
    if (printer.get_phase() == printer.phase2_func_defs && !sema.check(n)) {
      return;
    }
      ...
      if (n.has_name("_")) {
        if (n.has_wildcard_type()) {
          errors.emplace_back(
              ...
              "an object can have an anonymous name or an anonymous type, but "
              "not both at the same type (rationale: if '_ := f();' were "
              "allowed to keep the returned object alive, that syntax would be "
              "dangerously close to '_ = f();' to discard the returned object, "
              "and such importantly opposite meanings deserve more than a "
              "one-character typo distance; and explicit discarding gets the "
              "nice syntax because it's likely more common)");
          return;
        }
